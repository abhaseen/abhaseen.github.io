...
const defaultColor = "rgba(92, 235, 223, [opacity])";
const nightColor = "rgba(255, 255, 255, [opacity])";
...
export default class NodeGarden {
  ...
  resize() {
    ...
    if (this.nightMode) {
      this.ctx.fillStyle = nightColor.replace("[opacity]", "1");
    } else {
      this.ctx.fillStyle = defaultColor.replace("[opacity]", "1");
    }
  ...
  render(start, time) {
    ...
    for (let i = 0; i < this.nodes.length - 1; i++) {
      ...
      for (let j = i + 1; j < this.nodes.length; j++) {
        ...
        this.ctx.beginPath();
        if (this.nightMode) {
          this.ctx.strokeStyle = nightColor.replace(
            "[opacity]",
            `${opacity < 1 ? opacity : 1}`
          );
        } else {
          this.ctx.strokeStyle = defaultColor.replace(
            "[opacity]",
            `${opacity < 1 ? opacity : 1}`
          );
        }
